...
void Satlike::allocate_memory()
{
	...
	clause_max_weight = new long long[malloc_clause_length];
	clause_sum_weight = new long long[malloc_clause_length]; 
	clause_max_weight_id = new int[malloc_clause_length];
	...
	unsat_count = new int[malloc_clause_length];//added by jly
...
void Satlike::free_memory()
{
	...
	delete[] clause_sum_weight;
	delete[] clause_max_weight_id;
...
void Satlike::build_instance(char *filename)
{
	...
	for (c = 0; c < num_clauses; c++)
	{
		...
		clause_max_weight[c] = 1;//init. added by jly
		clause_sum_weight[c] = 0;//init. added by jly
		clause_max_weight_id[c] = -1;//init. added by jly
	...
	while (getline(infile, line2))
	{
		...
		for (i = 0; i < clause_lit_count[c]; ++i)
		{
			...
			if (clause_lit[c][i].weight != 1&& cc ==true) {cc=false;}
			if (clause_lit[c][i].weight > clause_max_weight[c]){// added by jly
				clause_max_weight[c] = clause_lit[c][i].weight;
				clause_max_weight_id[c] = i;
			}
			if (temp_lit[i] > 0)
...
void Satlike::init(vector<int> &init_solution)
{
	...
	for (c = 0; c < num_clauses; c++)
	{
		...
		if (org_clause_weight[c] == top_clause_weight)
		{
			...
			org_unit_weight[c] = ave_soft_weight;
...
int Satlike::pick_var()
{
	...
	if (hardunsat_stack_fill_pointer > 0)
	{
		if ((rand() % 2)  == 1)
			...
			sel_c = hardunsat_stack[0];
			if(hardunsat_stack_fill_pointer<10)
			    for(int ii=1;ii<hardunsat_stack_fill_pointer;ii++){
				int clause=hardunsat_stack[ii];
				if(unsat_count[clause]>unsat_count[sel_c])
					sel_c=clause;
			    }
			else{
			    for(int ii=1;ii<10;ii++){
			        int clause_=rand()%hardunsat_stack_fill_pointer;
			        if(unsat_count[clause_]>unsat_count[sel_c])
					sel_c=clause_;
			    }
			}
...
void Satlike::flip(int flipvar)
{
	...
	update_hardunsatcount();//added by jly20221027
}
...
void Satlike::local_search_with_decimation(vector<int> &init_solution, char *inputfile)
{
	...
	if(num_clauses < 2 * num_vars && num_clauses<100000){
	...
	else if(num_vars<400000 && num_clauses<400000){
		for (tries = 1; tries < max_tries; ++tries)
		{
				deci.init(local_opt_soln,best_soln,unit_clause,unit_clause_count,clause_lit_count,clause_sum_weight,clause_max_weight,clause_max_weight_id,clause_true_lit_thres);
			if(cc) deci.unit_prosess_cc();
			else deci.unit_prosess();
...
void Satlike::increase_weights()
{
	...
	//if(rand()%15==4)
	//for (int i = 1; i <= num_vars; ++i)
	//cout<<"var "<<i<<":"<<score[i]<< " "<<sscore[i]<<endl;
}
