...
class Decimation
{
  public:
    ...
    void free_memory();
    void init(int *ls_local_opt, int *ls_global_opt, lit *ls_unit_clause, int ls_unit_clause_count, int *ls_clause_lit_count,  long long *ls_clause_sum_lit_weight, long long *ls_clause_max_lit_weight, int *ls_clause_max_lit_weight_id, int *ls_clause_cardinality);
    void push_unit_clause_to_queue(lit tem_l);
    void assign(int v, int sense);
    void assign_cc(int v, int sense);
    void remove_unassigned_var(int v);
    void hunit_propagation();
    void sunit_propagation();
    void random_propagation();
    void hunit_propagation_cc();
    void sunit_propagation_cc();
    void random_propagation_cc();
    void unit_prosess();
    void unit_prosess_cc();
    ...
    long long *clause_max_lit_weight;
    long long *clause_sum_lit_weight;
    int *clause_max_lit_weight_id;
};
...
void Decimation::make_space(int max_c, int max_v)
{
    ...
    clause_cardinality = new int[max_c];

    clause_max_lit_weight=new long long[max_c];
    clause_sum_lit_weight=new long long[max_c];
    clause_max_lit_weight_id=new int[max_c];
}
...
void Decimation::free_memory()
{
    ...
    delete[] clause_cardinality;
    delete[] clause_max_lit_weight;
    delete[] clause_sum_lit_weight;
    delete[] clause_max_lit_weight_id;
}

void Decimation::init(int *ls_local_opt, int *ls_global_opt, lit *ls_unit_clause, int ls_unit_clause_count, int *ls_clause_lit_count, long long *ls_clause_sum_lit_weight, long long *ls_clause_max_lit_weight, int *ls_clause_max_lit_weight_id, int *ls_clause_cardinality)
{
    int v;
    ...
    for (int i = 0; i < num_clauses; ++i)
    {
        ...
        clause_cardinality[i] = ls_clause_cardinality[i];
        clause_delete[i] = 0;
        clause_max_lit_weight[i] = ls_clause_max_lit_weight[i];
        clause_sum_lit_weight[i] = ls_clause_sum_lit_weight[i];
        clause_max_lit_weight_id[i] = ls_clause_max_lit_weight_id[i];
    }
...
void Decimation::assign_cc(int v, int sense)
{
    int c, l;
    ...
    for (int i = 0; i < var_lit_count[v]; ++i)
    {
        ...
        {
            //clause_delete[c] = 1;
            clause_cardinality[c]--;
            if (clause_cardinality[c] == 0){
            ...
            else
            {
                ...
                if (clause_lit_count[c] == clause_cardinality[c])
                {
                    for (int j = 0; j < clause_lit_count[c]; ++j)
                    {
                        push_unit_clause_to_queue(clause_lit[c][j]);
                    }
                }
            }
            continue;
...
void Decimation::assign(int v, int sense)
{
    ...
    for (int i = 0; i < var_lit_count[v]; ++i)
    {
        ...
        if (sense == var_lit[v][i].sense)
        {
            //clause_delete[c] = 1;
            //clause_cardinality[c]--;
            clause_cardinality[c]-=var_lit[v][i].weight;
            clause_sum_lit_weight[c]-=var_lit[v][i].weight;

            if (clause_cardinality[c] <= 0)
                clause_delete[c] = 1;
            else //delete var in c
            {    
                ...
                if(clause_lit[c][clause_lit_count[c]].var_num == v){// update new clause max weight of c 
                    clause_max_lit_weight[c] = 0;
                    clause_max_lit_weight_id[c] = -1;
                    for (int j = 0; j < clause_lit_count[c]; j++){
                        if(clause_lit[c][j].weight > clause_max_lit_weight[c]){
                            clause_max_lit_weight[c]=clause_lit[c][j].weight;
                            clause_max_lit_weight_id[c] =j; 
                        }
                    }
                }
                if (clause_sum_lit_weight[c]-clause_max_lit_weight[c] < clause_cardinality[c]){
                    int maxid=clause_max_lit_weight_id[c];
                    if(maxid>=0)push_unit_clause_to_queue(clause_lit[c][maxid]);
                }
                if (clause_sum_lit_weight[c] <= clause_cardinality[c])
                {
                    for (int j = 0; j < clause_lit_count[c]; ++j)
                    {
                        push_unit_clause_to_queue(clause_lit[c][j]);
...
void Decimation::hunit_propagation_cc()
{
    int v, c, sense;
    v = hunit_clause_queue[hunit_beg_pointer].var_num;
    sense = hunit_clause_queue[hunit_beg_pointer].sense;
...
void Decimation::sunit_propagation_cc()
{
    int v, c, sense;
    v = sunit_clause_queue[sunit_beg_pointer].var_num;
    sense = sunit_clause_queue[sunit_beg_pointer].sense;
...
void Decimation::random_propagation_cc()
{
    int v, sense;
    v = unassigned_var[rand() % unassigned_var_count];
    sense = choose_sense(v);
...
void Decimation::unit_prosess_cc()
{
    while (unassigned_var_count > 0)
    {
        if (hunit_beg_pointer != hunit_end_pointer)
        {
            hunit_propagation_cc();
        }
        else if (sunit_beg_pointer != sunit_end_pointer)
        {
            sunit_propagation_cc();
        }
        else
        {
            random_propagation_cc();
        }
    }
}
