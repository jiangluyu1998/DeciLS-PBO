...
void Satlike::allocate_memory()
{
	...
	clause_sum_weight = new int[malloc_clause_length]; 
	...
	unsat_count = new int[malloc_clause_length];//added by jly20221025
...
int Satlike::pick_var()
{
	...
	{
		if (score[v] + sscore[v] + sscore[v] > score[best_var] + sscore[best_var]+ sscore[best_var])
			best_var = v;
		else if (score[v] + sscore[v]  + sscore[v] == score[best_var] + sscore[best_var]+ sscore[best_var])
		{
			if (time_stamp[v] < time_stamp[best_var])
				best_var = v;
		}
